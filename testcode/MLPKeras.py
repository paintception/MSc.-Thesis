...
dataframe = pandas.read_csv("Datasets/BiggerDataset.txt")
...
def run_model():
	# create model
	model = Sequential()
	model.add(Dense(768, input_dim=12, init='normal', activation='tanh'))
...
def main():
	...
	model.fit(X_train,y_train, nb_epoch=200, batch_size=5, verbose=1, validation_data=(X_test, y_test))
